...
@section('content')
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 用户列表管理 <span
                class="c-gray en">&gt;</span> 用户列表 <a class="btn btn-success radius r btn-refresh"
                                                      ...
                                                      title="刷新"
                                                      onclick="location.replace('{{URL::asset('/admin/mryh/mryhUser/index')}}');"><i
                    class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="page-container">

        <div class="text-c">
            <form  id="search_form" action="{{URL::asset('/admin/mryh/mryhUser/index')}}" method="post" class="form-horizontal">
                {{csrf_field()}}
            ...
            <table class="table table-border table-bordered table-bg table-sort">
                ...
                @foreach($datas as $data)
                    <tr class="text-c">
                        ...
                        <td>
                            {{isset($data->join_num)?$data->join_num:'--'}}
                        </td>
                        <td>
                            {{isset($data->success_num)?$data->success_num:'--'}}
                        </td>
                        <td>
                            {{isset($data->fail_num)?$data->fail_num:'--'}}
                        </td>
                        <td>
                            {{isset($data->join_day_num)?$data->join_day_num:'--'}}
                        </td>
                        <td>
                            {{isset($data->work_num)?$data->work_num:'--'}}
                        </td>
                        ...
                        <td class="td-manage">
                            <a style="text-decoration:none"
                               onClick="showJoins('{{$data->user->id}}','{{$data->user->nick_name}}')"
...
@section('script')
    <script type="text/javascript">
         ...
         * 2018-09-21
         */
        function showJoins(user_id, nick_name) {
            creatIframe("{{URL::asset('/admin/mryh/mryhJoin/index')}}?user_id=" + user_id, "参赛明细-" + nick_name);
        }
