...
@section('content')
    ...
    <nav class="breadcrumb">大赛管理 <a class="btn btn-success radius r btn-refresh"
                                    ...
                                    title="刷新"
                                    onclick="location.replace('{{URL::asset('/admin/vote/voteActivity/edit')}}?item={{$item}}&id={{isset($data)?$data->id:''}}');"><i
    ...
    <div class="page-container">
        ...
        {{--通过item控制显示选项--}}
        @if($item=='0')
            @include('admin.vote.voteActivity.item0')
        @endif
        @if($item=='1')
            @include('admin.vote.voteActivity.item1')
        @endif
        @if($item=='2')
            @include('admin.vote.voteActivity.item2')
        @endif
        @if($item=='3')
            @include('admin.vote.voteActivity.item3')
        @endif
        @if($item=='4')
            @include('admin.vote.voteActivity.item4')
        @endif
        @if($item=='5')
            @include('admin.vote.voteActivity.item5')
        @endif
        @if($item=='6')
            @include('admin.vote.voteActivity.item6')
        @endif
        @if($item=='7')
            @include('admin.vote.voteActivity.item7')
        @endif
        @if($item=='8')
            @include('admin.vote.voteActivity.item8')
        @endif
        @if($item=='9')
            @include('admin.vote.voteActivity.item9')
        @endif
        @if($item=='10')
            @include('admin.vote.voteActivity.item10')
        @endif
        @if($item=='11')
            @include('admin.vote.voteActivity.item11')
        @endif
    </div>
...
@section('script')
    <script type="text/javascript">
        ...
        function selectItem(item) {
            ...
            location.replace('{{URL::asset('/admin/vote/voteActivity/edit')}}?id={{$data->id}}&item=' + item);
        }
