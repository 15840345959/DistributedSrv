...
@section('content')
    ...
    <div class="page-container">
        <form class="form form-horizontal" id="form-edit">
            ...
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>模式：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <span class="select-box" style="width: 400px;">
                        <select id="mode" name="mode" class="select">
                            <option value="">请选择</option>
                            @foreach(\App\Components\Utils::VOTE_AD_MODE_VAL as $key=>$value)
                                <option value="{{$key}}" {{$data->mode==$key?'selected':''}}>{{$value}}</option>
                            @endforeach
                        </select>
                    </span>
                </div>
            ...
            <div class="row cl item">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    ...
                    <div style="font-size: 12px;margin-top: 10px;" class="text-gray">
                        *请上传800*600或者600*550比例尺寸图片，因为广告图包括首页、排名页和投票弹出页广告，所以应考虑具体尺寸进行上传，要求上传大比例尺寸图片，技术团队将进行智能剪裁，上传大尺寸图片可以保证图片的清晰度。
                    </div>
                </div>
            </div>
            <div class="row cl item">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input id="url" name="url" type="text" class="input-text"
                           value="{{ isset($data->url) ? $data->url : '' }}" placeholder="请输入跳转链接">
                </div>
            </div>
        ...
        $(function () {
            ...
            $("#form-edit").validate({
                ...
                submitHandler: function (form) {
                    ...
                    $(form).ajaxSubmit({
                        type: 'POST',
                        url: "{{ URL::asset('/admin/vote/voteAD/edit')}}",
