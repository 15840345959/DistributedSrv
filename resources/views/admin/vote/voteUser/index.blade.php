...
@section('content')
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 参赛选手管理 <span
                class="c-gray en">&gt;</span> 参赛选手列表 <a class="btn btn-success radius r btn-refresh"
                                                        ...
                                                        title="刷新"
                                                        onclick="location.replace('{{URL::asset('/admin/vote/voteUser/index')}}');"><i
                    class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="page-container">

        <div class="text-c">
            <form id="search_form" action="{{URL::asset('/admin/vote/voteUser/index')}}" method="post"
                        ...
                        <td>
                            <div>
                                <a class="c-primary" style="text-decoration:none"
                                   onclick="creatIframe('{{URL::asset('/admin/vote/voteActivity/edit')}}?id={{$data->activity->id}}', '大赛信息-{{$data->activity->name}}');">{{$data->activity->name}}</a>
                            </div>
                            <div class="mt-5">
                                @if($data->user)
                        ...
                        <td class="td-manage">
                            <div>
                                ...
                                <a title="编辑" href="javascript:;"
                                   onclick="edit('编辑参赛选手-{{$data->name}}','{{URL::asset('/admin/vote/voteUser/edit')}}?id={{$data->id}}',{{$data->id}})"
                            ...
                            <div class="mt-5">
                                <a class="c-primary" style="text-decoration:none"
                                   onclick="creatIframe('{{URL::asset('/admin/vote/voteUser/info')}}?id={{$data->id}}', '选手信息-{{$data->name}}');">
                                    详细信息
                                </a>
                            </div>
                            <div class="mt-5">
                                <a title="编辑" href="javascript:;" class="c-primary"
                                   onclick="layer_show('查看二维码', '{{route('voteUser.qrcode', ['id' => $data->id])}}', 400, 400)"
...
@section('script')
    <script type="text/javascript">
        ...
        /*参赛选手-删除*/
        function del(obj, id) {
            layer.alert('不能删除选手，否则将导致数据混乱，请联系技术团队 TerryQi负责');
        }
        ...
        function stop(obj, id) {
            ...
            layer.confirm('确认要停用吗？', function (index) {
                ...
                //从后台设置参赛选手状态
                vote_voteUser_setStatus('{{URL::asset('')}}', param, function (ret) {
                    if (ret.status == true) {
        ...
        function audit(obj, id, audit_status) {
            ...
            layer.confirm('确认要进行审核吗？', function (index) {
                ...
                //从后台设置参赛选手状态
                vote_voteUser_setAuditStatus('{{URL::asset('')}}', param, function (ret) {
