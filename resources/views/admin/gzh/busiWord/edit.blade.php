        ...
        <form class="form form-horizontal" id="form-edit">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    ...
                    <select class="select" name="template_type" id="template_type" size="1"
                            onchange="selectTemplateType();">
                        <option value="TEMPLATE_CUSTOM">自定义模板</option>
                        @foreach(\App\Components\Utils::NORMAL_TEMPLATE_VAL as $key=>$value)
                            <option id="{{$key}}"
                                    value="{{$key}}" {{$data->template_id==$key?'selected':''}}>{{$value}}</option>
                        @endforeach
                    </select>
                    </span>
                </div>
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input id="template_id" name="template_id" type="text" class="input-text"
                           value="{{ isset($data->template_id) ? $data->template_id : '' }}" placeholder="请定义模板ID">
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input id="name" name="name" type="text" class="input-text"
                           value="{{ isset($data->name) ? $data->name : '' }}" placeholder="请输入业务场景">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>回复顺序：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input id="seq" name="seq" type="number" class="input-text" style="width: 100%;"
                           value="{{ isset($data->seq) ? $data->seq : '0' }}" placeholder="请定义回复顺序">
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <div>
                        <span class="c-999">可以回复多条消息，回复消息的顺序由回复顺序决定，值越大越靠前</span>
                    </div>
                </div>
            </div>
            <div class="row cl">
            ...
            <div class="row cl item text-item">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    ...
                    <div>
                        <span class="c-999">针对大赛有一些变量，调整时需要联系技术组处理</span>
                    </div>
                    <div>
                        <span class="c-primary">选手ID:{vote_user_id}  选手姓名:{vote_user_name} 选手当前票数:{vote_user_vote_num} 选手当前排名:{vote_user_pm}</span>
                    </div>
                </div>
            </div>
...
@section('script')
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#form-edit").validate({
                ...
                submitHandler: function (form) {
                    ...
                    $(form).ajaxSubmit({
                        type: 'POST',
                        url: "{{ URL::asset('/admin/gzh/busiWord/edit')}}",
        ...
        function selectTemplateType() {
            console.log("selectTemplateType");
            var template_type = $("#template_type").val();
            console.log("template_type:" + template_type);
            //清空模板id和场景
            $("#template_id").val('');
            $("#name").val('');
            //根据条件设置template_id，非自定义模板
            if (template_type != "TEMPLATE_CUSTOM") {
                $("#template_id").val(template_type);
                //业务场景名称
                $("#name").val($("#" + template_type).text());
            }
        }
