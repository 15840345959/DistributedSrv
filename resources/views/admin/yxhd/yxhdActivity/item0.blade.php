<form class="form form-horizontal" id="form-edit">
        ...
        <div class="formControls col-xs-8 col-sm-9">
            <span class="select-box" style="width: 200px;">
                <select id="type" name="type" class="select">
                    ...
                    @foreach(\App\Components\Utils::YXHD_TYPE_VAL as $key=>$value)
    ...
    <div class="row cl item c-999">
        ...
        <div class="formControls col-xs-8 col-sm-9">
            <div>
                <span>活动类型共有：转盘活动等</span>
            </div>
        </div>
    </div>
    <div class="row cl">
        <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>活动代码：</label>
        <div class="formControls col-xs-8 col-sm-9">
            <input id="code" name="code" type="text" class="input-text" style="width: 200px;"
                   value="{{ isset($data->code) ? $data->code : '' }}"
                   placeholder="活动代码不能重复">
        </div>
    </div>
    <div class="row cl item c-999">
        <label class="form-label col-xs-4 col-sm-2"></label>
        <div class="formControls col-xs-8 col-sm-9">
            <div>
                <span>在公众号中回复活动代码，可以推送活动的缩略图，活动编码也可以是中文</span>
            </div>
        </div>
    </div>
    <div class="row cl">
        <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>需要积分：</label>
        <div class="formControls col-xs-8 col-sm-9">
            <input id="join_score" name="join_score" type="number" class="input-text"
                   value="{{ isset($data->join_score) ? $data->join_score : '1' }}"
                   placeholder="请输入目标参与天数">
        </div>
    </div>
    <div class="row cl item c-999">
        <label class="form-label col-xs-4 col-sm-2"></label>
    ...
    $(function () {
        ...
        $("#form-edit").validate({
            ...
            submitHandler: function (form) {
                ...
                $(form).ajaxSubmit({
                    type: 'POST',
                    url: "{{ URL::asset('/admin/yxhd/yxhdActivity/edit')}}",
                    success: function (ret) {
                        console.log(JSON.stringify(ret));
                        if (ret.result) {
                            layer.msg('保存成功', {icon: 1, time: 1000});
                            location.replace('{{URL::asset('/admin/yxhd/yxhdActivity/edit')}}?item={{$item}}&id=' + ret.ret.id);
                        } else {
