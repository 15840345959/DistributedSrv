...
@section('content')
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 参赛选手管理 <span
                class="c-gray en">&gt;</span> 参赛选手信息 <a class="btn btn-success radius r btn-refresh"
                                                        ...
                                                        title="刷新"
                                                        onclick="location.replace('{{route('team.activity.voteUserInfo')}}?id={{$data->id}}');"><i
                    class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="page-container">

        <div class="text-l">
            <button type="submit" class="btn btn-primary" id="" name=""
                    onclick="creatIframe('{{route('team.activity.voteRecord')}}?vote_user_id={{$data->id}}', '投票明细-{{$data->name}}');">
                投票明细
            </button>
            <button type="submit" class="btn btn-primary" id="" name=""
                    onclick="creatIframe('{{route('team.activity.voteShareRecord')}}?vote_user_id={{$data->id}}', '分享明细-{{$data->name}}');">
                分享明细
            </button>
            <button type="submit" class="btn btn-primary" id="" name=""
                    onclick="creatIframe('{{route('team.activity.voteFollowRecord')}}?vote_user_id={{$data->id}}', '关注明细-{{$data->name}}');">
                关注明细
            </button>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <a class="c-primary" style="text-decoration:none"
                               onclick="creatIframe('{{route('team.activity.edit')}}?id={{$data->activity->id}})', '大赛信息-{{$data->activity->name}}');">
