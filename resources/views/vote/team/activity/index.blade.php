...
@section('content')
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 大赛管理 <span
                class="c-gray en">&gt;</span> 大赛列表 <a class="btn btn-success radius r btn-refresh"
                                                      ...
                                                      title="刷新"
                                                      onclick="location.replace('{{route('team.activity.index')}}');"><i
                    class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="page-container">

        <div class="text-c">
            <form action="{{URL::asset('activity')}}" method="post" class="form-horizontal">
                {{csrf_field()}}
        ...
        <div class="cl pd-5 bg-1 bk-gray mt-20">
            <span class="l">
                 <a href="javascript:;"
                    onclick="edit('新建大赛','{{route('team.activity.edit')}}')"
                        ...
                        <td class="td-manage">
                            ...
                            <div>
                                ...
                                <a title="编辑" href="javascript:;"
                                   onclick="edit('编辑大赛-{{$data->name}}','{{route('team.activity.edit')}}?id={{$data->id}}',{{$data->id}})"
    ...
    <script type="text/javascript">
        ...
        function stop(obj, id) {
            ...
            layer.confirm('确认要停用吗？', function (index) {
                ...
                //从后台设置大赛状态
                vote_team_activity_setStatus('{{route('team.activity.setActivityStatus')}}', param, function (ret) {
                    if (ret.result == true) {
                        layer.msg('已停用', {icon: 5, time: 1000});
                        location.replace('{{route('team.activity.index')}}');
                    } else {
        ...
        function copy(activity_id) {
            layer.confirm('确认要复制该大赛吗？', function (index) {
                ...
                var index = layer.load(2, {time: 10 * 1000}); //加载
                //从后台设置大赛状态
                vote_team_activity_copy('{{route('team.activity.copy')}}', param, function (ret) {
                    if (ret.result == true) {
                        layer.msg('复制成功', {icon: 1, time: 1000});
                        //打开新的编辑窗口
                        creatIframe("{{route('team.activity.edit')}}?id=" + ret.ret.id, "编辑大赛-" + ret.ret.name);
                        location.replace('{{route('team.activity.index')}}');
                    } else {
                        layer.msg(ret.message, {icon: 5, time: 2000});
                    }
        ...
        function exportPrizeStatements(id) {
            ...
            window.location.href = '{{route('team.activity.prizeStatements')}}?id=' + id + '&_token=' + '{{csrf_token()}}'
        }
